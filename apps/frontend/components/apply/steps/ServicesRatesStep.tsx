  ...
  return (
    <div>
      ...
      {form.rateMinDollars !== '' && form.rateMaxDollars !== '' && maxDollars <= minDollars && (
        <p className="mt-2 text-xs text-red-600">Max must be greater than min.</p>
      )}

      <StepActions onBack={onBack} onNext={onNext} nextLabel="Next: Review" nextDisabled={!canContinue} />
    </div>
  );
}
